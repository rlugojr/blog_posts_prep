...
> [RefSeq](https://www.ncbi.nlm.nih.gov/refseq/) contains a database of well-characterized genes.
...
p <- ggplot(gene_table, aes(x = factor(DB), y = no_genes, label = no_genes)) + 
  ...
  labs(title = "Number of genes in database", y = "Count", x = "")

p + geom_text(size=6, vjust=1.5)
```

- Number of Entrez IDs: `r length(unique(ENTREZID))`
- Number of Ensembl IDs: `r length(unique(ENSEMBL))`
- Number of gene symbols: `r length(unique(HGNC))`
- Number of Refseq IDs: `r length(unique(REFSEQ))`
