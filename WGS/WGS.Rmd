...
```{r echo=TRUE, eval=FALSE, message=FALSE, warning=FALSE, fig.width=16, fig.height=8, fig.align="center", cache=TRUE}
diff_table_timeline <- diff_table_bind[which(diff_table_bind$Indicator.Name == measure), ]
diff_table_timeline <- diff_table_timeline[which(diff_table_timeline$Country.Code %in% wmap_countries_df_final$gu_a3), ]

library(tidyr)
diff_table_timeline_gather <- gather(diff_table_timeline, year, value, X1960:X2015)

ggplot(diff_table_timeline_gather, aes(x = year, y = value, color = Country.Name, group = Country.Code)) +
  geom_point() +
  geom_line() +
  my_theme()
```
