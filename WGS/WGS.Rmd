...
```{r echo=TRUE, eval=FALSE, message=FALSE, warning=FALSE, fig.width=16, fig.height=8, fig.align="center", cache=TRUE}
year_table <- diff_table_bind[which(diff_table_bind$Country.Code %in% wmap_countries_df_final$gu_a3), ]
...
```{r echo=TRUE, eval=FALSE, message=FALSE, warning=FALSE, fig.width=16, fig.height=8, fig.align="center", cache=TRUE}
map <- left_join(subset(wmap_countries_df_final, !continent == "Antarctica"), subset(year_table_last_val, Indicator.Name == measure), by = c("gu_a3" = "Country.Code"))
...
```{r echo=TRUE, eval=FALSE, message=FALSE, warning=FALSE, fig.width=16, fig.height=8, fig.align="center", cache=TRUE}
ggplot(map, aes(x = economy, y = log2(unlist.last_val.), fill = income_grp, color = income_grp)) +
...
```{r echo=TRUE, eval=FALSE, message=FALSE, warning=FALSE, fig.width=16, fig.height=8, fig.align="center", cache=TRUE}
aggregate(log2(unlist.last_val.) ~ economy * income_grp, data = map, mean)

subs <- map[!duplicated(map$gu_a3), c("gu_a3", "name", "economy", "income_grp", "unlist.last_val.")]
arrange(subset(subs, !is.na(unlist.last_val.)), economy, income_grp)
```

```{r echo=TRUE, eval=FALSE, message=FALSE, warning=FALSE, fig.width=16, fig.height=8, fig.align="center", cache=TRUE}
ggplot(map, aes(x = log2(unlist.last_val.), fill = income_grp, color = income_grp)) +
  my_theme() +
  geom_density(alpha=0.3)
```

```{r echo=TRUE, eval=FALSE, message=FALSE, warning=FALSE, fig.width=16, fig.height=8, fig.align="center", cache=TRUE}
ggplot(map, aes(x = log2(unlist.last_val.), fill = economy, color = economy)) +
  my_theme() +
  geom_density(alpha=0.3)
```

```{r echo=TRUE, eval=FALSE, message=FALSE, warning=FALSE, fig.width=16, fig.height=8, fig.align="center", cache=TRUE}
kw1 <- kruskal.test(log2(unlist.last_val.) ~ economy, data = map)
str(kw1)

kruskal.test(log2(unlist.last_val.) ~ income_grp, data = map)
