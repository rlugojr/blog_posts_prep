...
```{r echo = TRUE, message = FALSE, warning = FALSE, cache=FALSE, fig.width = 10, fig.height = 10, fig.align = "center"}
library(tidyr)
links <- homologs_human_table[, -grep("sapiens", colnames(homologs_human_table))] %>%
  gather(other, value, oanatinus_gene_ensembl:cfamiliaris_gene_ensembl)
links <- links[-which(links$value == 0), ]
links$GENEID <- "human"
nodes <- colnames(homologs_human_table[, -grep("sapiens", colnames(homologs_human_table))])

links2 <- co_occurrence
nodes2 <- colnames(co_occurrence)
```
