...
df_BP <- as.data.frame(go_enrich_BP_hsapiens_gene_ensembl)

ontology <- as.list(GOBPPARENTS)
goIds <- unique(df_BP$ID)[1:100]
goIds <- goIds[goIds %in% names(ontology)]
...
goTerms_df$color <- ifelse(rownames(goTerms_df) %in% goIds, "red", "skyblue2")

V(goGraph)$color <- goTerms_df$color
V(goGraph)$label <- goTerms

E(goGraph)$color <- ifelse(E(goGraph)$type == "is_a", "slategrey",
...
```{r echo=TRUE, message=FALSE, warning=FALSE, fig.width=25, fig.height=25, fig.align="center", cache=FALSE}
goGraph <- simplify(goGraph, remove.multiple = T, remove.loops = T, edge.attr.comb = c(weight = "sum", type = "ignore"))
