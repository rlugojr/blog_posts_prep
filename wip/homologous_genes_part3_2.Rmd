...
for (i in 1:nrow(datasets)){
  ...
  write.table(genes, paste("genes", species, sep = "_"), row.names = F, col.names = F, quote = F)
...
# because yeast didn't work above, I'm leaving it out
...
Because there are usually so many enriched GO terms, it is difficult to compare them. Using the dotplot() function from clusterProfiler produced plots which were too wide, too small and too difficult to comprehend. Thus, I'm resorting to a non-R solution here: Bingo and Cytoscape.
...
getAmigoTree(goIDs, set3, filename = "example", picType = "png",
...
amiTree <- getAmigoTree(unique(plot_df$ID), filename = "example", picType = "dot", modeType = "amigo", saveResult = FALSE)

amiTree_file <- readAmigoDot(object = amiTree)
